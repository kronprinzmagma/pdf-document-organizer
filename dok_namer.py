...
# Cloud-Modell und Pricing (USD per million tokens)
...
# Thread-Lock für Zielwahl + Verschieben: verhindert, dass zwei Worker
# gleichzeitig denselben freien Zielnamen ermitteln (rename überschreibt
# auf POSIX stillschweigend — ohne Lock droht Datenverlust).
_move_lock = threading.Lock()

# llama3.2 hybrid-Anonymisierung: deterministisch, mit hartem Timeout.
LLAMA_TIMEOUT_S    = 30.0
LLAMA_NUM_PREDICT  = 2048

# Erlaubte Platzhalter, die llama3.2 im Anonymisierungs-Pass erzeugen darf.
# Synchron zu README und CLAUDE.md halten. Alles ausserhalb dieses Sets wird
# als Halluzination gewertet und separat berichtet.
...
REDACTION_PATTERNS = [
    ...
    # Swiss AHV number ohne Punkte: 13 Ziffern beginnend mit 756 (maschinelle Formate)
    (re.compile(r'\b756\d{10}\b'), "[AHV]"),
    ...
    # Suffix-Liste deckt auch seltenere CH-Formen ab (Vorstadt, Quai, Halde, …);
    # false positives sind unkritisch — über-redaktieren ist die sichere Richtung.
    (re.compile(
        r'[A-ZÄÖÜ][a-zA-Zäöüÿ]+'
        r'(?:strasse|gasse|weg|allee|platz|ring|str\.?|vorstadt|quai|halde|rain|hof|matte)'
        r'[\s\-]?\d+[a-z]?[,\s]+'
    ...
    # Anrede + 1–3 grossgeschriebene Wörter (Empfängernamen ohne Titel).
    # Bewusst NACH [BEHANDLER], damit «Frau Dr. Müller» zuerst als Titel+Name greift.
    (re.compile(
        r'\b(?:Herrn?|Frau|Familie|Fr[äa]ulein)\s+'
        r'[A-ZÄÖÜ][a-zA-ZäöüÄÖÜßéè\-]+(?:\s+[A-ZÄÖÜ][a-zA-ZäöüÄÖÜßéè\-]+){0,2}'
    ), "[NAME]"),
]

# Zeilen, die wie ein Personenname aussehen (1–3 grossgeschriebene Wörter,
# keine Ziffern), direkt über einer redaktierten Adresse → Empfängername.
_NAME_LINE_RE = re.compile(
    r'^[A-ZÄÖÜ][a-zA-ZäöüÄÖÜßéè\-]+(?:\s+[A-ZÄÖÜ][a-zA-ZäöüÄÖÜßéè\-\.]+){0,2}$'
)

# Marker, die auf Organisationen statt Personen hindeuten — solche Zeilen
# bleiben stehen, damit die Absender-Extraktion nicht leidet.
# Exakte Tokens (kurz, als Substring zu fehleranfällig) vs. Wortbestandteile.
_ORG_TOKEN_MARKERS = (
    "ag", "gmbh", "sa", "sarl", "ltd", "inc", "kg", "co", "postfach",
)
_ORG_SUBSTR_MARKERS = (
    "amt", "kasse", "bank", "versicherung", "gemeinde", "kanton",
    "stadt", "direktion", "verwaltung", "stiftung", "verein",
    "genossenschaft",
)


def _looks_like_person_line(line: str) -> bool:
    """Heuristik: Zeile besteht nur aus einem Personennamen (kein Organisationsname)."""
    stripped = line.strip()
    if not stripped or not _NAME_LINE_RE.match(stripped):
        return False
    words = [w.strip(".").lower() for w in stripped.split()]
    if any(w in _ORG_TOKEN_MARKERS for w in words):
        return False
    return not any(marker in w for w in words for marker in _ORG_SUBSTR_MARKERS)


def _redact_names_near_addresses(text: str) -> tuple[str, int]:
    """Bis zu zwei Namenszeilen direkt über einem [ADRESSE]-Platzhalter durch [NAME] ersetzen.

    Adressblöcke in Briefen tragen den Empfängernamen meist unmittelbar über
    der Strassenzeile — ohne Anrede oder Titel entgeht er den Regex-Patterns.
    """
    lines = text.split("\n")
    replaced = 0
    for i, line in enumerate(lines):
        if "[ADRESSE]" not in line:
            continue
        for j in (i - 1, i - 2):
            if j < 0:
                break
            if _looks_like_person_line(lines[j]):
                lines[j] = "[NAME]"
                replaced += 1
            else:
                break
...
def classify_error(exc: Exception) -> str:
    ...
    Primär anhand des Exception-Typs; Message-Substrings nur als Fallback.
    Wichtig: Dateipfade in Fehlermeldungen enden fast immer auf '.pdf' —
    deshalb darf 'pdf' nie gegen die Message gematcht werden.
    """
    name = type(exc).__name__.lower()
    msg = str(exc).lower()
    if "authentication" in name or "api_key" in msg or "auth_token" in msg:
        return "auth-missing"
    if "timeout" in name or "timed out" in msg:
        return "timeout"
    if "ratelimit" in name or "rate limit" in msg or "429" in msg:
        return "rate-limit"
    if "overloaded" in name or "overloaded" in msg or "529" in msg:
        return "overloaded"
    if "json" in name or "validation" in name:
        return "parse-error"
    if "pdf" in name or "eof marker" in msg:
...
def move_renamed(directory: Path, filename: str, src: Path) -> Path:
    """Datei unter neuem Namen kollisionsfrei verschieben (thread-sicher).

    Zielwahl und rename passieren unter einem Lock: rename überschreibt auf
    POSIX stillschweigend, zwei parallele Worker mit identischem Vorschlag
    würden sich sonst gegenseitig Dateien löschen.
    """
    with _move_lock:
...
def move_unchanged(directory: Path, src: Path) -> Path:
    """Datei unter Originalnamen kollisionsfrei verschieben (thread-sicher)."""
    with _move_lock:
        dest = unique_original_destination(directory, src.name)
        src.rename(dest)
    return dest
...
def load_categories(
    ...
    Lookup-Reihenfolge:
      1. explizit uebergebener path (--config)
      2. categories.yaml im Quellordner (source_folder)
      3. categories.yaml im aktuellen Arbeitsverzeichnis – nur als Fallback,
         mit Hinweis auf stdout, damit klar ist, dass der Quellordner leer war.
    Wenn nichts gefunden wird, kommen die englischen Defaults zurueck.
    """
    ...
    if path is not None:
        ...
        candidate = source_folder / "categories.yaml"
        if candidate.exists():
            chosen = candidate
        else:
            cwd_candidate = Path.cwd() / "categories.yaml"
            if cwd_candidate.exists():
                print(
                    f"  Hinweis: keine categories.yaml im Quellordner ({source_folder}); "
                    f"benutze Fallback im aktuellen Verzeichnis: {cwd_candidate}"
                )
                chosen = cwd_candidate
    else:
        chosen = Path.cwd() / "categories.yaml"
...
def run_eval(
    ...
    if include_claude:
        if all(case.get("cloud_safe") for case in cases):
            ...
            configs.append((CLAUDE_MODEL, "claude"))
        else:
...
def redact(text: str, blocklist: tuple[str, ...] = ()) -> tuple[str, dict[str, int]]:
    ...
    # Empfängernamen-Heuristik: Namenszeilen direkt über redaktierten Adressen
    text, near_address = _redact_names_near_addresses(text)
    if near_address:
        counts["[NAME]"] = counts.get("[NAME]", 0) + near_address
...
REGEX_SENSITIVE_PATTERNS: list[tuple[str, re.Pattern]] = [
    # AHV: strukturiertes Format ist spezifisch genug (mit und ohne Punkte)
    ("AHV",
     re.compile(r'756\.\d{4}\.\d{4}\.\d{2}|(?<!\d)756\d{10}(?!\d)')),
...
def anonymize_with_llama(text: str) -> tuple[str, dict[str, int], bool]:
    """
    Rohen PDF-Text an llama3.2 via Ollama zur LLM-Anonymisierung senden.
    Gibt (anonymized_text, {placeholder: count}, truncated) zurück.
    truncated=True heisst: das Modell hat das Token-Limit erreicht und der
    Output ist abgeschnitten — der Text darf so nicht weiterverwendet werden.
    """
    try:
        import ollama as ollama_lib
    except ImportError:
    ...
    prompt = (
        ...
        "  [IBAN]      for bank account numbers and IBANs\n"
        ...
        "  [BEHANDLER] for academic or medical titles together with the person name (e.g. 'Dr. Müller')\n"
        "\n"
    ...
    try:
        client = ollama_lib.Client(timeout=LLAMA_TIMEOUT_S)
        response = client.chat(
            model="llama3.2",
            messages=[{"role": "user", "content": prompt}],
            options={"temperature": 0, "num_predict": LLAMA_NUM_PREDICT},
        )
    except Exception as e:
        error_msg = str(e)
        if "timeout" in error_msg.lower() or "timed out" in error_msg.lower():
            print(f"  Fehler: llama3.2-Anonymisierung lief in Timeout (> {LLAMA_TIMEOUT_S}s)")
    ...
    # done_reason == "length": Output am Token-Limit abgeschnitten
    ...
    for m in re.finditer(r'\[[A-ZÄÖÜ]+\]', anonymized):
        label = m.group(0)
        if label in ALLOWED_PLACEHOLDERS:
            counts[label] = counts.get(label, 0) + 1
        else:
            hallucinated[label] = hallucinated.get(label, 0) + 1

    if hallucinated:
        summary = ", ".join(f"{n}× {label}" for label, n in hallucinated.items())
        print(f"  Hinweis: llama3.2 erzeugte unbekannte Platzhalter (Halluzination): {summary}")

    return anonymized, counts, truncated
...
def analyze_hybrid(
    ...
    # Merken, ob der Rohtext überhaupt PII-Muster enthielt — Basis für den No-op-Check
    raw_had_pii = any(pattern.search(raw_text) for pattern, _ in REDACTION_PATTERNS)

    t0 = time.perf_counter()
    anonymized, placeholder_counts, truncated = anonymize_with_llama(raw_text)
    timings["anonymize"] = time.perf_counter() - t0

    if truncated:
        return None, None, placeholder_counts, "REVIEW", "llama-output-abgeschnitten", timings

    # No-op-Check: Rohtext hatte PII-Muster, aber llama hat keinen einzigen
    # Platzhalter gesetzt → Anonymisierung vermutlich wirkungslos (z.B. Text
    # unverändert zurückgegeben). Namen kann die Regex-Verifikation nicht
    # prüfen, deshalb geht die Datei in review/ statt an die Cloud.
    if raw_had_pii and sum(placeholder_counts.values()) == 0:
        return None, None, placeholder_counts, "REVIEW", "llama-anonymisierung-wirkungslos", timings

    t0 = time.perf_counter()
    # Defense-in-Depth: Regex-Redaktion auch NACH llama — fängt alles, was
    # das LLM übersehen hat (kostet nichts, läuft rein lokal).
    anonymized, regex_counts = redact(anonymized, blocklist)
    for label, count in regex_counts.items():
        placeholder_counts[label] = placeholder_counts.get(label, 0) + count
...
def process_file(
    ...
    def technical_skip(reason: str, cost: float = 0.0, elapsed: float = 0.0) -> tuple[str, float, float, str, str, dict[str, float], str, str]:
        emit("  → Moved unchanged to 'error/'")
        move_unchanged(error_dir, path)
    ...
    if engine == "hybrid":
        ...
        if verify_verdict == "REVIEW":
            ...
            move_unchanged(review_dir, path)
    ...
    if info.confidence == "low":
        ...
        emit("  → Moved unchanged to 'unrecognized/'")
        move_unchanged(unrecognized_dir, path)
    ...
    dest = move_renamed(category_dir, final, path)
...
def write_report(
    ...
    if engine in ("ollama", "hybrid"):
        lines.append("| llama3.2 / llava (lokal) | CHF 0.0000 |")
...
def main():
    ...
    categories = load_categories(args.config, source_folder=folder)
    engine  = args.engine
    auto    = args.auto
    ...
    def _record_result(outcome: tuple) -> None:
        """Log-Eintrag schreiben und Zähler aktualisieren (gemeinsam für beide Modi)."""
        nonlocal renamed, unrecognized, review, skipped, total_cost
        result, cost, elapsed, suggested, confidence, phase_timings, error_reason, done_category, fname = outcome
        total_cost += cost
    ...
    if workers > 1:
        ...
        for i, pdf in enumerate(pdfs, 1):
            _record_result(_process_one((i, pdf)))
    ...
    if engine == "ollama":
        print("  Total API cost      : CHF 0.00 (local model)")
    elif engine == "hybrid":
