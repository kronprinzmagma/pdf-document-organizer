...
> **About this repository:** Public, sanitised version of a private working repo that the author uses on real personal correspondence (tax, banking, insurance). The code is identical; this repo ships with a synthetic test set instead of real PDFs. Synced automatically after every push to the private repo.

<!-- HERO-VISUAL — bitte hier ein Bild einfügen (stärkster visueller Hebel):
     Animiertes GIF eines echten interaktiven Laufs mit SYNTHETISCHEN Daten
     (Redactions-Zeile, Tokens/Cost, Confidence, Suggested filename, Accept y/n).
     1. GIF ablegen unter  docs/demo-run.gif  (Pfad zur Sync-Whitelist in
        scripts/sync_to_public.py hinzufügen).
     2. Folgende Zeile einkommentieren:
![dok-namer im interaktiven Lauf](docs/demo-run.gif)
-->
...
| Swiss AHV social security number (dotted and 13-digit form) | `[AHV]` |
...
| Salutation + capitalised words (Herr/Frau/Familie …) | `[NAME]` |
| Recipient-name lines directly above a redacted address | `[NAME]` |
...
```bash
# Folder mode only — to process a single file, place it in a folder
